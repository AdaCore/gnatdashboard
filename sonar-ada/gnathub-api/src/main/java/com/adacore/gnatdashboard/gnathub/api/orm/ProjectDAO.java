...
import com.adacore.gnatdashboard.gnathub.api.codepeer.CodePeerSeverity;
...
public class ProjectDAO {
  ...
  private final RowMapper<IssueRecord> issueRowMapper = new RowMapper<IssueRecord>() {
    ...
    public IssueRecord mapRow(ResultSet resultSet) throws SQLException {
      ...
      if ("codepeer".equalsIgnoreCase(toolName)) {
        try {
  ...
  public final File getFileById(final Integer resourceId) {
    ...
    try {
      files = connector.query(SQLQueries.resourceById(resourceId), fileRowMapper);
    } catch (final SQLException why) {
      log.error("Database error", why);
      return null;
    }
