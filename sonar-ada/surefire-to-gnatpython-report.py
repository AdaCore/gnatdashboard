...
"""Convert a Maven Surefire test report into a GNATpython test report

The Maven Surefire plugin generates XML reports for each testsuite executed.
ANOD test framework expects JSON reports, generated by GNATpython based on the
...
See the GNATpython documentation for information about the expected output
(:func:`gnatpython.internal.gaia.report.json_report`).
...
def list_surefire_xml_reports(input_dir):
    ...
    :param input_dir: the directory to scan for XML report files
    :type input_dir: str
...
def parse_testcase_node(testcase_node):
    ...
    :param testcase_node: the XML node to parse
    :type testcase_node: xml.dom.Node
    :return: the parsed testcase report
    :rtype: SureFireTestcase, SureFireFailure | None, SureFireError
    """
...
def generate_gnatpython_report(output_dir, testsuites):
...
def surefire_to_gnatpython_report(input_dir, output_dir):
    ...
    # Generate GNATpython reports
    generate_gnatpython_report(output_dir, testsuites)
    return 0
...
if __name__ == '__main__':
    ...
    main = argparse.ArgumentParser(
        description='Convert MavenSurefire test report into GNATpython report')
    main.add_argument(
        '-i', '--input', metavar='SUREFIRE_REPORT_DIR', required=True,
        help='the input Surefire test reports directory')
    main.add_argument(
        '-o', '--output', metavar='GNATPYTHON_REPORT_dir', required=True,
        help='the output GNATpython test reports directory')
    main.add_argument(
    ...
    sys.exit(surefire_to_gnatpython_report(args.input, args.output))
