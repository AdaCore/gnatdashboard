...
# built documents.
#
with open(os.path.join(TOP, 'VERSION'), 'r') as version_file:
    # The full version, including alpha/beta/rc tags.
    release = version_file.read().strip()

    match = VERSION_RE.match(release)
    assert match, '%s: unexpected version: %s' % (version_file.name, release)

    # The short X.Y version.
    version = '%s.%s' % match.group('major', 'minor')
...
html_static_path = [os.path.join('..', 'themes', 'adacore', 'static')]
...
htmlhelp_basename = 'GNATdashboard-doc'
...
# [howto/manual]).
latex_documents = [
    ('index', 'GNATdashboard.tex', u'GNATdashboard Documentation', u'AdaCore',
     'manual'),
]
...
man_pages = [
    ('index', 'gnatdashboard', u'GNATdashboard Documentation', [u'AdaCore'], 1)
]
...
def setup(app):
    """Customizes documentation generation"""
